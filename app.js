...
const STARRED = [
  { ticker: "H100", name: "NVIDIA H100 80GB SXM", sector: "NVIDIA", price: 2.49, mu: -0.18, sigma: 0.45 },
  { ticker: "B200", name: "NVIDIA Blackwell B200", sector: "NVIDIA", price: 5.85, mu: -0.05, sigma: 0.55 },
  { ticker: "H200", name: "NVIDIA H200 141GB", sector: "NVIDIA", price: 3.45, mu: -0.12, sigma: 0.42 },
  { ticker: "A100", name: "NVIDIA A100 80GB", sector: "NVIDIA", price: 1.15, mu: -0.28, sigma: 0.35 },
  { ticker: "L40S", name: "NVIDIA L40S 48GB", sector: "NVIDIA", price: 0.95, mu: -0.15, sigma: 0.3 },
  { ticker: "4090", name: "NVIDIA RTX 4090 24GB", sector: "NVIDIA", price: 0.42, mu: -0.1, sigma: 0.4 },
  { ticker: "MI300X", name: "AMD Instinct MI300X", sector: "AMD", price: 2.1, mu: 0.05, sigma: 0.5 },
  { ticker: "MI325X", name: "AMD Instinct MI325X", sector: "AMD", price: 3.2, mu: 0.1, sigma: 0.55 },
  { ticker: "TPUv5p", name: "Google TPU v5p", sector: "Google", price: 4.2, mu: -0.08, sigma: 0.38 },
  { ticker: "TRN2", name: "AWS Trainium2", sector: "AWS", price: 1.85, mu: -0.2, sigma: 0.36 },
];
  ...
  "4090": [
    {
      ...
      "author": "@indie_ai",
      "handle": "5h",
      "text": "$4090 still the indie-builder gold standard. 24GB + cheap = chef's kiss."
    },
    {
      "sent": "neg",
      "author": "@datacenter_pm",
      "handle": "11h",
      "text": "$4090 EULA prohibits datacenter use. Not actually deployable at scale."
    }
  ],
  "H100": [
    {
      ...
      "author": "@cluster_ops",
      "handle": "2h",
      "text": "$H100 hourly spot back to $2.49 — H200 cannibalizing demand."
    },
    {
      "sent": "neg",
      "author": "@infra_quant",
      "handle": "8h",
      "text": "$H100 still 4x worse perf/W vs $B200. Hyperscalers refresh inevitable."
    },
    {
      "sent": "pos",
      "author": "@inference_co",
      "handle": "1d",
      "text": "$H100 sweet spot for inference workloads. We're long for 2026."
    }
  ],
  "B200": [
    {
      ...
      "author": "@ai_capex",
      "handle": "1h",
      "text": "$B200 deployment ramping faster than guidance. Hyperscaler queues months deep."
    },
    {
      "sent": "pos",
      "author": "@datacenter_pm",
      "handle": "5h",
      "text": "$B200 FP4 economics are 10x H100 on Llama-class inference. Game over short-term."
    },
    {
      "sent": "neu",
      "author": "@hwarch",
      "handle": "12h",
      "text": "$B200 power density is forcing liquid cooling everywhere. Real bottleneck is grid."
    }
  ],
  "H200": [
    {
      ...
      "author": "@llm_train",
      "handle": "4h",
      "text": "$H200 memory bandwidth is the unlock for 70B+ inference. We're scaling."
    },
    {
      "sent": "neu",
      "author": "@cloud_arb",
      "handle": "10h",
      "text": "$H200 spot inside $H100 + $B200 spread. Fair-value parking spot."
    }
  ],
  "A100": [
    {
      ...
      "author": "@gpu_resell",
      "handle": "3h",
      "text": "$A100 fading fast. Inference moving to $L40S, training to $H200+."
    },
    {
      "sent": "neu",
      "author": "@academic_ai",
      "handle": "1d",
      "text": "$A100 still the price-per-FLOP champion for research workloads."
    }
  ],
  "L40S": [
    {
      ...
      "author": "@inference_co",
      "handle": "2h",
      "text": "$L40S the right answer for diffusion inference. We're doubling our fleet."
    },
    {
      "sent": "neu",
      "author": "@hosters",
      "handle": "8h",
      "text": "$L40S availability good, prices stable. Steady utilization."
    }
  ],
  "MI300X": [
    {
      ...
      "author": "@amd_bull",
      "handle": "1h",
      "text": "$MI300X 192GB HBM3 is the inference monster. Microsoft committed."
    },
    {
      "sent": "neu",
      "author": "@rocm_skeptic",
      "handle": "9h",
      "text": "$MI300X hardware great, ROCm tooling still 18 months behind CUDA."
    },
    {
      "sent": "pos",
      "author": "@open_compute",
      "handle": "1d",
      "text": "$MI300X price-performance lapping NVIDIA for inference. Long $AMD."
    }
  ],
  "MI325X": [
    {
      ...
      "author": "@amd_bull",
      "handle": "3h",
      "text": "$MI325X with 288GB HBM3e — single-node 405B inference. Crushed forecast."
    },
    {
      "sent": "neu",
      "author": "@infra_quant",
      "handle": "1d",
      "text": "$MI325X TCO promising but production volumes 1/10 of NVIDIA. Allocation > price."
    }
  ],
  "TPUv5p": [
    {
      ...
      "author": "@goog_ai",
      "handle": "2h",
      "text": "$TPUv5p training Gemini Ultra at fraction of NVIDIA cost. Vertical integration paying off."
    },
    {
      "sent": "neu",
      "author": "@cloud_arb",
      "handle": "11h",
      "text": "$TPUv5p only available via GCP — vendor lock priced in."
    }
  ],
  "TRN2": [
    {
      ...
      "author": "@aws_re",
      "handle": "4h",
      "text": "$TRN2 Anthropic deployment hitting numbers. AWS finally has a real silicon story."
    },
    {
      "sent": "neg",
      "author": "@silicon_takes",
      "handle": "1d",
      "text": "$TRN2 ecosystem still thin. Most teams won't port off CUDA for 20% savings."
    }
  ]
};
...
const STRATEGIES = [
  {
    ...
    apply: (ctx) => {
      ...
      if (s20 == null || s50 == null) return { signal: "MID RANGE", detail: "Not enough history for 50-period MA." };
      if (s20 > s50 * 1.005) return { signal: "LOWER COST",  detail: `20-period (${s20.toFixed(2)}) above 50-period (${s50.toFixed(2)}) — rising trend.` };
      if (s20 < s50 * 0.995) return { signal: "HIGHER COST", detail: `20-period (${s20.toFixed(2)}) below 50-period (${s50.toFixed(2)}) — falling trend.` };
      return { signal: "MID RANGE", detail: `20-period ≈ 50-period — consolidation.` };
    },
  },
  {
    id: "rsi_mr",
  ...
  },
  {
    ...
    apply: (ctx) => {
      ...
      if (ratio > 1.3 && expectedReturn > 0.01) return { signal: "LOWER COST", detail: `Up/down ratio ${ratio.toFixed(2)}x, Mean ${(expectedReturn * 100).toFixed(1)}%, Pr(up) ${(probUp * 100).toFixed(0)}%.` };
      if (ratio < 0.8 && expectedReturn < -0.01) return { signal: "HIGHER COST", detail: `Up/down ratio ${ratio.toFixed(2)}x, Mean ${(expectedReturn * 100).toFixed(1)}%, Pr(up) ${(probUp * 100).toFixed(0)}%.` };
  ...
  },
  {
    ...
    apply: (ctx) => {
      ...
      if (s.score > 0.25)  return { signal: "LOWER COST",  detail: `Net +${(s.score * 100).toFixed(0)} — note balance positive.` };
      if (s.score < -0.15) return { signal: "HIGHER COST", detail: `Net ${(s.score * 100).toFixed(0)} — note balance negative.` };
...
function combineSignals(results) {
  ...
  if (avg > 0.35)  return { signal: "LOWER COST",    detail: "Majority positive." };
  if (avg < -0.35) return { signal: "HIGHER COST",   detail: "Majority negative." };
...
function computeCohortKpis(stocks) {
  ...
  const pv = stocks.reduce((a, s) => a + s.price, 0);
  const prevPv = stocks.reduce((a, s) => a + s.prevClose, 0);
  const pnl = pv - prevPv;
...
const kpiDefs = () => {
  ...
  return [
    ...
    { id: "kpi-er",     label: "Expected price change", value: k.expectedReturn,  fmt: (v) => pctFmt(v),    mod: k.expectedReturn >= 0 ? "positive" : "negative", valueMod: k.expectedReturn >= 0 ? "up" : "down", sub: `${state.horizon}-step · scenario` },
    { id: "kpi-lower",    label: "95% cost bound",         value: k.lowerBound,           fmt: (v) => pctFmt(v),    mod: "negative", valueMod: "down",           sub: "5th percentile" },
    { id: "kpi-stability", label: "Price stability",          value: k.stability,          fmt: (v) => v.toFixed(2), mod: "accent",                              sub: "Normalised dispersion" },
...
function renderDetail(onComplete) {
  ...
  $("#detail-subtitle").textContent = `Vendor: ${stock.sector} · Spot $/hr ${priceFmt(stock.price)} (${pctFmt(stock.change)}) · Drift μ=${(stock.mu * 100).toFixed(1)}%, vol σ=${(stock.sigma * 100).toFixed(1)}%.`;
...
function renderChart(mc, stock, onComplete) {
  ...
  const s = mc.summary;
  const captionText = `${mc.nPaths.toLocaleString()} seeded scenario paths over ${mc.days} trading days. Expected price change ${pctFmt(s.expectedReturn)}, median ${pctFmt(s.medianReturn)}, 95% CI ${pctFmt(s.ci95Low)} to ${pctFmt(s.ci95High)}, probability of gain ${(s.probUp * 100).toFixed(0)}%.`;
...
function renderSentiment(stock) {
  ...
  const posts = s.posts.length ? s.posts : [{ sent: "neu", author: "@scenario", handle: "now", text: "No recent scenario notes for this SKU." }];
...
function renderSummary() {
  ...
  const agg = `${buy} LOWER COST · ${uns} BALANCED · ${sell} HIGHER COST across ${all.length} SKUs.`;
...
// ========== Hyperscaler capex + token economics panel ==========
function renderExtraPanel() {
  const container = document.getElementById("extra-content");
  if (!container || !state.stocks) return;
  const w = 720, h = 280, padL = 80, padR = 12, padT = 14, padB = 22;
  const capex = [
    { name: "Microsoft / Azure", value: 88, color: "#00d9ff" },
    { name: "Google / Alphabet", value: 75, color: "#5ce8ff" },
    { name: "Meta",              value: 65, color: "#ff9933" },
    { name: "Amazon / AWS",      value: 105, color: "#00d9ff" },
    { name: "Oracle",            value: 25, color: "#5ce8ff" },
    { name: "ByteDance",         value: 20, color: "#ff66cc" },
  ];
  const maxCapex = Math.max(...capex.map(c => c.value));
  const rowH = (h - padT - padB - 60) / capex.length;
  const barX = padL, barMaxW = w - padL - padR - 40;
  const capexBars = capex.map((c, i) => {
    const y = padT + i * rowH;
    const barW = (c.value / maxCapex) * barMaxW;
    return `
      <text x="${padL - 6}" y="${y + rowH / 2 + 3}" text-anchor="end" fill="#d6dce4" font-size="10" font-family="JetBrains Mono, monospace">${c.name}</text>
      <rect x="${barX}" y="${y + 4}" width="${barW}" height="${rowH - 8}" fill="${c.color}" opacity="0.75"/>
      <text x="${barX + barW + 6}" y="${y + rowH / 2 + 3}" fill="#00d9ff" font-size="10" font-family="JetBrains Mono, monospace">${c.value}B</text>
    `;
  }).join("");
  // Token economics: tokens/$ per SKU (synthetic but ratio-correct)
  const tokens = state.stocks.map(s => {
    const tpd = (1.5e6 / s.price) * (s.ticker.startsWith("B") ? 3.2 : s.ticker === "MI300X" || s.ticker === "MI325X" ? 1.8 : 1.0);
    return { ticker: s.ticker, tpd };
  });
  const tpdMax = Math.max(...tokens.map(t => t.tpd));
  const tpdRowY = padT + capex.length * rowH + 18;
  const tpdBars = tokens.map((t, i) => {
    const segW = (w - padL - padR) / tokens.length;
    const x = padL + i * segW;
    const barH = (t.tpd / tpdMax) * 32;
    return `
      <rect x="${x + 2}" y="${tpdRowY + 32 - barH}" width="${segW - 4}" height="${barH}" fill="#5ce8ff" opacity="0.8"/>
      <text x="${x + segW / 2}" y="${tpdRowY + 48}" fill="#d6dce4" font-size="9" text-anchor="middle" font-family="JetBrains Mono, monospace">${t.ticker}</text>
    `;
  }).join("");
  container.innerHTML = `
    <svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="Hyperscaler 2026 AI capex (${capex.map(c => c.name + ' ' + c.value + ' billion').join(', ')}) and tokens-per-dollar by SKU.">
      <text x="${padL - 6}" y="${padT - 2}" fill="#7f8693" font-size="9" text-anchor="end" font-family="JetBrains Mono, monospace">2026 AI CAPEX</text>
      ${capexBars}
      <line x1="${padL}" x2="${w - padR}" y1="${tpdRowY - 4}" y2="${tpdRowY - 4}" stroke="#191c23"/>
      <text x="${padL - 6}" y="${tpdRowY + 6}" fill="#7f8693" font-size="9" text-anchor="end" font-family="JetBrains Mono, monospace">TOK/$ </text>
      ${tpdBars}
    </svg>
  `;
...
function renderExtra3Panel() {
  ...
  if (!container) return;
  const data = [{"name":"λ jump intensity","units":"+50%","down":-7.8,"up":8.4},{"name":"σ diffusion","units":"+25%","down":-4.1,"up":5.2},{"name":"α secular drift","units":"+20%","down":-3.2,"up":3.6},{"name":"β supply elasticity","units":"+25%","down":-1.8,"up":1.4},{"name":"u_t utilization","units":"+10pp","down":-1.5,"up":2.1},{"name":"Jump magnitude E[J]","units":"+5pp","down":-0.9,"up":1.1}];
  ...
  const bars = data.map((d, i) => {
    ...
    const upBar = `<rect x="${xZero}" y="${y}" width="${ux.toFixed(1)}" height="${bh}" fill="#00d9ff" fill-opacity="0.72"/>`;
    ...
    const upText = `<text x="${xZero + ux + 4}" y="${y + bh / 2 + 4}" fill="#00d9ff" font-size="10" font-family="JetBrains Mono, monospace">+${d.up.toFixed(1)}%</text>`;
...
function renderExtra4Panel() {
  ...
  if (!container) return;
  const X = {"name":"σ","values":[0.25,0.35,0.45,0.55,0.65]};
  const Y = {"name":"λ","values":[0.2,0.4,0.6,0.8,1,1.2]};
  const label = "12-mo log-return CI95 width";
  const surface = (x, y) => Math.min(1, x * 0.6 + y * 0.5 + x * y * 0.4);
  const palette = ["#1a2029", "#0099b3", "#00d9ff", "#5ce8ff", "#ff3366"];
  const glyphs = [".", "·", "=", "▲", "■"];
